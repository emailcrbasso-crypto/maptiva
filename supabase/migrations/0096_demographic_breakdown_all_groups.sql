-- =====================================================================
-- 0096_demographic_breakdown_all_groups.sql
--
-- Auditoria externa (Flexmetal v2, Marcelo Pezzutti): no bloco "Nível
-- detalhado" do corte demográfico, só aparece "Equipe" (Direto). Pares
-- (6 avaliadores) e Clientes internos (12) somem, mesmo tendo 3 ou
-- mais respondentes — o BD diz que nenhum grupo deveria ser suprimido
-- (PARAMETROS, linhas 24 e 25).
--
-- Causa: nivel_detalhe vem só de assignments.relationship_detail
-- (migration 0072), que só é preenchido para relationship_code =
-- 'subordinate' (Direto/Indireto) — para 'peer'/'client'/'manager'/
-- 'manager_superior' o valor é null, e a query filtra "where
-- nivel_detalhe is not null", descartando essas linhas inteiras.
--
-- Fix: quando relationship_detail é null, cai para um rótulo derivado
-- de relationship_code (Pares, Clientes internos, Chefe direto,
-- Liderança Superior) — mesma terminologia já usada no resto do
-- relatório. O frontend (NIVEL_DETALHE_VALUE_LABEL) já repassa esses
-- valores sem alteração (só traduz 'Direto'/'Indireto').
-- =====================================================================
      ...
      select
        ...
        coalesce(
          ...
          case a.relationship_code
            when 'peer'             then 'Pares'
            when 'client'           then 'Clientes internos'
            when 'manager'          then 'Chefe direto'
            when 'manager_superior' then 'Liderança Superior'
            else null
          end
        )                                  as nivel_detalhe,
        r.score                           as score
  ...
  return (
    with evaluator_scores as (
      ...
      where a.cycle_id          = p_cycle_id
        ...
        and a.status            = 'completed'
        and r.score is not null
      group by a.evaluator_cycle_participant_id, a.evaluated_cycle_participant_id, pe.metadata_json, a.relationship_code, a.relationship_detail
    ),
    dims as (
      select 'sexo'          as dimension, sexo          as value, avaliador_score from evaluator_scores where sexo          is not null
      union all
