-- =====================================================================
-- 0036_assignment_context_competency_name.sql
--
-- Adiciona competency_name a cada questão no retorno de
-- get_assignment_context, para que a RespondPage possa agrupar
-- perguntas visualmente por competência sem um segundo roundtrip.
--
-- A função é SECURITY DEFINER e acessível a anon — seguro porque
-- só expõe o nome da competência (dado de apresentação).
-- =====================================================================
...
begin
  ...
  -- Perguntas em ordem, incluindo nome da competência
  select coalesce(
    jsonb_agg(
      jsonb_build_object(
        'id',              q.id,
        ...
        'competency_name', co.name         -- null quando sem competência vinculada
      )
