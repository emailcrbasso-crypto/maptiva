-- =====================================================================
-- 0068_demographic_sexo_and_cycle_level.sql
--
-- Duas adições ao corte demográfico:
--
--   1. Nova dimensão 'sexo' (metadata_json ->> 'sexo') no corte por
--      participante — mesmo padrão de geracao/cargo/tempo_casa.
--
--   2. Novo RPC app.get_cycle_demographic_breakdown(p_cycle_id) —
--      mesmo corte, mas agregado sobre TODOS os avaliados do ciclo
--      (não um participante isolado). Usado na Síntese Executiva da
--      Liderança do relatório do ciclo.
--
-- Ambos seguem a mesma regra de anonimato: exclui autoavaliação e
-- aplica n_minimum_default do template.
-- =====================================================================

-- ── 1. get_participant_demographic_breakdown — adiciona 'sexo' ──────
...
-- ── 2. get_cycle_demographic_breakdown — mesmo corte, ciclo inteiro ──
...
begin
  ...
  return (
    with evaluator_scores as (
      -- Score médio (todas as competências, todos os avaliados do ciclo)
      -- por par (avaliador, avaliado) não-self.
      select
        a.evaluator_cycle_participant_id,
...
create or replace function public.get_cycle_demographic_breakdown(p_cycle_id uuid)
returns jsonb language plpgsql security definer set search_path = public, pg_temp
as $$ begin return app.get_cycle_demographic_breakdown(p_cycle_id); end$$;

grant execute on function public.get_cycle_demographic_breakdown(uuid) to authenticated;
