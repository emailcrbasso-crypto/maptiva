-- 0047_fix_dpa_dashboard.sql
--
-- Corrige get_dpa_dashboard:
-- Antes: buscava o projeto com "tenant_id = app.current_tenant_id()" dentro de
--        uma função security definer — app.current_tenant_id() pode ser NULL
--        quando chamada nesse contexto, fazendo a consulta não encontrar nada.
-- Depois: busca o projeto apenas pelo id, depois verifica autorização pelo
--         tenant_memberships usando auth.uid() diretamente.
