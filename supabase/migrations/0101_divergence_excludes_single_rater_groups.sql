-- =====================================================================
-- 0101_divergence_excludes_single_rater_groups.sql
--
-- Bug encontrado ao testar a 0100: a Divergência estava incluindo
-- Chefe direto e Liderança Superior (grupos de 1 pessoa só) na
-- comparação de "mais alto"/"mais baixo", porque a HAVING copiava a
-- isenção de n-mínimo usada em get_question_scores (onde faz sentido
-- sempre mostrar a nota do chefe, mesmo sendo 1 pessoa).
--
-- Mas pra Divergência a regra do BD é outra: "Grupos com 3 ou mais
-- avaliadores (Equipe, Pares e Clientes internos)" — chefe direto e
-- liderança superior não entram na comparação, porque comparar contra
-- um grupo de 1 pessoa exporia a nota individual dela como extremo
-- (mesmo problema já corrigido em "Atenção especial").
--
-- Remove a isenção: agora só entram grupos com count(*) >= v_n_min.
-- =====================================================================
