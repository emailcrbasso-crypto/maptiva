-- =====================================================================
-- 0081_fix_n_minimum_and_consultant_notes.sql
--
-- Duas correções sérias em app.compute_scores, encontradas numa
-- auditoria externa e confirmadas no código:
--
-- 1) N-mínimo contava RESPOSTAS, não avaliadores distintos.
--    visibility_status usava `count(*) >= v_n_min` sobre linhas de
--    responses — um único avaliador respondendo 3 perguntas da mesma
--    competência já batia n_minimum=3 sozinho, liberando (visible) um
--    grupo formado por 1 pessoa só. Corrigido para
--    `count(distinct a.evaluator_cycle_participant_id) >= v_n_min`.
--    Isso é o dado que score_snapshots.visibility_status alimenta —
--    usado por comments_published, ReportPDF, GAP/blind-spot, etc.
--
-- 2) "Recalcular scores" apagava as notas do consultor.
--    compute_scores faz DELETE + INSERT em participant_result_profiles;
--    consultant_notes (0070) mora na mesma linha e nunca era
--    preservado — qualquer recálculo zerava o campo. Corrigido:
--    captura as notas existentes antes do DELETE e restaura depois do
--    INSERT.
--
-- IMPORTANTE: esta migration só corrige a FUNÇÃO — os ciclos já
-- fechados continuam com os snapshots antigos (possivelmente errados)
-- até compute_scores ser chamado de novo para cada um. Reprocessar
-- ciclos reais de cliente é uma ação separada, deliberada.
-- =====================================================================
...
begin
  ...
  -- Preserva as notas do consultor antes de apagar os profiles —
  -- consultant_notes (0070) não é recalculado, é conteúdo editorial.
  ...
  -- ── Snapshots ────────────────────────────────────────────────────
  -- N-mínimo agora conta AVALIADORES DISTINTOS, não linhas de resposta
  -- (um avaliador respondendo várias perguntas da mesma competência não
  -- deve, sozinho, destravar a visibilidade de um grupo).
  ...
  -- Restaura as notas do consultor preservadas antes do recálculo.
